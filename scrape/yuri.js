...
const getYuri = async (page = 1, limit = 10) => {
  try {
    ...
    if (page <= 1) {
      url = `${BaseURL}/series/yuri`;
    } else {
      url = `${BaseURL}/series/yuri/page/${page}`;
    }
    ...
    element.each((index, el) => {
      ...
      divs.each((index, elm) => {
        ...
        if (title && image && id) {
          ...
          if (!uniqueSet.has(obj.id) && yuriArr.length < limit) {
