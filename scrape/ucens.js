...
const getUncensored = async (page = 1, limit = 10) => {
  try {
    ...
    if (page <= 1) {
      url = `${BaseURL}/series/uncensored`;
    } else {
      url = `${BaseURL}/series/uncensored/page/${page}`;
    }
    ...
    element.each((index, el) => {
      ...
      divs.each((index, elm) => {
        ...
        if (title && image && id) {
          ...
          if (!uniqueSet.has(obj.id) && uncensoredArr.length < limit) {
            uniqueSet.add(obj.id);
            uncensoredArr.push(obj);
          }
        }
      });
    });

    return uncensoredArr;
  } catch (error) {
    throw createError(500, error?.message);
  }
};

module.exports = { getUncensored };
