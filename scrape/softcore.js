...
const getSoftcore = async (page = 1, limit = 10) => {
  try {
    ...
    if (page <= 1) {
      url = `${BaseURL}/series/softcore`;
    } else {
      url = `${BaseURL}/series/softcore/page/${page}`;
    }
    ...
    element.each((index, el) => {
      ...
      divs.each((index, elm) => {
        ...
        if (title && image && id) {
          ...
          if (!uniqueSet.has(obj.id) && softcoreArr.length < limit) {
            uniqueSet.add(obj.id);
            softcoreArr.push(obj);
          }
        }
      });
    });
